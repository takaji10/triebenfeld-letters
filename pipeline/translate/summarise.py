...
def build():
    ...
    # 32 summaries over one of 345, and said nothing about it. Any rebuild that
    # produces fewer records than the file already holds is a scoping mistake
    # until someone says otherwise.
    if os.path.isfile(DEST):
        with open(DEST, encoding='utf-8') as f:
            had = len(yaml.safe_load(f) or {})
        if len(out) < had:
            sys.exit(f'refusing to write {DEST}: it holds {had} summaries and '
                     f'this run assembled {len(out)}. Something scoped the walk '
